...
class AccountMove(models.Model):
    ...
    def _get_base_url_password(self):
        """
        Fetches the base URL and session token used for Tripletex API authentication.
        :return: Tuple (base_url, token) or (None, None) if not found.
        """
        print('\n---------_get_base_url_password')
        session_token = self.env['tripletex.session.token'].search([], limit=1)
        return (self.env['ir.config_parameter'].sudo().get_param('ak_tripletex_auth.base_url'),
                session_token.token if session_token else None)

    def _get_base_url_password_headers(self):
        """
        Returns base URL, session token, and default headers for Tripletex API calls.
        :return: Tuple (base_url, token, headers)
        """
        print('\n---------_get_base_url_password_headers')
        base_url, token = self._get_base_url_password()
        headers = {'Content-Type': 'application/json', "Accept": "application/json"}

        print('\n---------self._get_base_url_password()', (base_url, token))
        print('\n---------sdd', (base_url, token, headers))

        return (base_url, token, headers)

    def _make_tripletex_request(self, url, method='get', data=None):
        """
         ...
         """
        print('\n=====_make_tripletex_request==')
        base_url, password, headers = self._get_base_url_password_headers()
        ...
        if method == 'get':
            ...
            print('\n---------response',response)
            print('\n---------headers',url,data,password,headers)
    ...
    def get_country_id(self, iso_alpha2_code):
        ...
        """
        print('\n---------get_country_id')
        base_url, password = self._get_base_url_password()
        if not base_url or not password:
            return None
        try:
            response = self._make_tripletex_request(f'{base_url}/country')
    ...
    def get_tripletex_currency_data(self, currency_name):
        ...
        """
        print('\n---------get_tripletex_currency_data')
        base_url, password, headers = self._get_base_url_password_headers()
        if not base_url or not password:
            return None
        try:
            response = self._make_tripletex_request(f'{base_url}/currency')
        except Exception as e:
            raise ValidationError(e)
        if response.status_code == 200:
    ...
    def action_post(self):
        ...
        for rec in self:
            ...
            contact_tripletex_id = rec._create_contact_if_needed(base_url)
            delivery_address = rec._get_delivery_address()
            print('\bnnnnnnnnnnnn', rec._create_or_get_customer(base_url, rec._get_customer_data()))
            data = {
                ...
                'orders': [{
                    ...
                    "currency": rec.get_tripletex_currency_data(rec.currency_id.name),  # Use the currency data
                    "orderDate": rec['invoice_date'].isoformat(),
                    "deliveryDate": rec['delivery_date'].isoformat() if rec[
                        'delivery_date'] else datetime.today().isoformat()
                }],
            }
            ...
            try:
                response = rec._make_tripletex_request(f'{base_url}/invoice', method='post', data=json.dumps(data))
            except Exception as e:
                raise ValidationError(e)
    ...
    def _create_or_get_customer(self, base_url, customer_data):
        ...
        if partner.company_type == 'company' and not partner.trip_customer:
            try:
                response = self._make_tripletex_request(f'{base_url}/customer', method='post',
    ...
    def _create_contact_if_needed(self, base_url):
        ...
        if partner.company_type == 'person' and partner.type == 'invoice' and partner.parent_id and partner.parent_id.trip_customer:
            if self.sale_order_id.partner_id.trip_customer and self.sale_order_id.partner_id.company_type == 'person':
                ...
                if contact_data:
                    try:
                        response = self._make_tripletex_request(f'{base_url}/contact', method='post',
    ...
    #     try:
    #       response = self._make_tripletex_request(f'{base_url}/order/orderGroup', method='post', data=json.dumps(payload))
    #     except Exception as e:
    #       raise ValidationError(e)
    #    if response.status_code == 201:
    ...
    def _create_tripletex_product_if_needed(self, base_url, product):
        ...
        if not product.tripletex_product:
            ...
            try:
                response = self._make_tripletex_request(f'{base_url}/product', method='post', data=json.dumps(product_data))
            except Exception as e:
                raise ValidationError(e)
            if response.status_code == 201:
