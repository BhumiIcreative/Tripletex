...
class AccountMove(models.Model):
    ...
    invoice_num = fields.Char(string="Tripletex Invoice Number", readonly=True)
    invoice = fields.Integer(string="Tripletex Invoice ID")
    is_hidden = fields.Boolean(
        default=False, string="Hidden", help="To see tripletex info on it"
    )
    ...
    tripletex_comment = fields.Text(string="Comment")
    ...
    def onchange_customer_id(self):
        ...
        """
        self.customer = (
            self.partner_id.trip_customer if self.partner_id else False
        )
    ...
    def make_tripletex_request(self, url, method="get", data=None):
        ...
        if method.lower() in ["get", "post"]:
            payload = data if method.lower() == "post" else None
    ...
    def get_country_id(self, iso_alpha2_code):
        ...
        if response.status_code == 200:
            ...
            raise ValidationError(
                _("Error getting country data from Tripletex: {}").format(
    ...
    def get_tripletex_currency_data(self, currency_name):
        ...
        if response.status_code == 200:
            ...
            raise ValidationError(
                _("Error getting currency data from Tripletex: {}").format(
                    response.text if response.text else "Unknown error"
                )
            )
    ...
    def action_post(self):
        ...
        for rec in self:
            if rec.journal_id.type == "sale" and rec.invoice_num:
                raise ValidationError(
                    _(
                        "Duplicate Invoice Not Allowed. Please create a Credit Note and Issue a New Invoice."
                    )
                )
            contact_tripletex_id = rec.create_contact_if_needed(base_url)
            data = {
                "invoiceDate": rec.invoice_date.isoformat()
                if rec.invoice_date
                else datetime.today().isoformat(),
                "invoiceDueDate": rec.invoice_date_due.isoformat()
                ...
                "orders": [
                    {
                        ...
                        "customer": {
                            "id": rec._create_or_get_customer(
                                base_url, rec._get_customer_data()
                            )
                        },
                        "contact": {"id": contact_tripletex_id}
                        if contact_tripletex_id
                        else None,
                        "orderLines": rec._prepare_orderlines(base_url),
                        "currency": rec.get_tripletex_currency_data(
                            rec.currency_id.name
                        ),
                        "orderDate": rec.invoice_date.isoformat()
                        if rec.invoice_date
                        else datetime.today().isoformat(),
                        "deliveryDate": rec.delivery_date.isoformat()
            ...
            try:
                response = rec.make_tripletex_request(
                    f"{base_url}/invoice", method="post", data=json.dumps(data)
                )
            except Exception as e:
                raise ValidationError(e)
            if response.status_code == 201:
                rec.write(
                    {
                        "invoice_num": response.json()["value"][
                            "invoiceNumber"
                        ],
    ...
    def _get_iso_alpha2_code(self):
        ...
        """
        country_record = self.env["res.country"].browse(
            self.partner_shipping_id.country_id.id
        )
        if not country_record.code:
            raise ValidationError(
                _("ISO Alpha-2 code not found for the shipping country.")
            )
    ...
    def _get_customer_data(self):
        ...
        return {
            ...
            "postalAddress": {
                ...
                "country": {
                    "id": self.get_country_id(partner.country_id.code)
                },
            },
        }
    ...
    def _create_or_get_customer(self, base_url, customer_data):
        ...
        if partner.company_type == "company" and not partner.trip_customer:
            try:
                response = self.make_tripletex_request(
                    f"{base_url}/customer",
                    method="post",
            ...
            if response.status_code == 201:
                partner.write(
                    {"trip_customer": response.json()["value"]["id"]}
                )
            else:
                raise ValidationError(
                    _("Error creating customer in Tripletex: {}").format(
                        response.text if response.text else "Unknown error"
                    )
                )
        elif (
            ...
            and not partner.parent_id.trip_customer
        ):
            try:
                response = self.make_tripletex_request(
                    f"{base_url}/customer",
            ...
            and partner.parent_id
            and partner.parent_id.trip_customer
        ):
            ...
            if self.sale_order_id.partner_id.company_type == "person":
                customer_id = (
                    self.sale_order_id.partner_id.parent_id.trip_customer
                )
        elif (
            partner.company_type == "person"
            and partner.parent_id
    ...
    def create_contact_if_needed(self, base_url):
        ...
        if (
            ...
            if (
                self.sale_order_id.partner_id.trip_customer
                and self.sale_order_id.partner_id.company_type == "person"
            ):
                contact_tripletex_id = (
                    self.sale_order_id.partner_id.trip_customer
                )
            elif (
                self.sale_order_id.partner_id.trip_customer
                and self.sale_order_id.partner_id.company_type == "company"
            ):
                contact_tripletex_id = None
            else:
                first_name, last_name = (
                    (
                        self.sale_order_id.partner_id.name.split(" ", 1)
                        + [None]
                    )[:2]
                    if isinstance(self.sale_order_id.partner_id.name, str)
                    and " " in self.sale_order_id.partner_id.name
                    else (self.sale_order_id.partner_id.name, None)
                )
                contact_data = {
                    "firstName": first_name,
                    "lastName": last_name,
                    "email": self.sale_order_id.partner_id.email or None,
                    "phoneNumberMobile": self.sale_order_id.partner_id.phone
                    or None,
                    "trip_customer": {
                        "id": self.sale_order_id.partner_id.parent_id.trip_customer
                    }
                    if self.sale_order_id.partner_id.parent_id.trip_customer
                    else None,
                }
                if contact_data:
                    try:
                        ...
                        raise ValidationError(e)
                    if response.status_code == 201:
                        self.sale_order_id.partner_id.write(
                            {"trip_customer": response.json()["value"]["id"]}
                        )
                    else:
                        raise ValidationError(
                            _(
                                "Error creating contact in Tripletex: {}"
                            ).format(
                                response.text
                                if response.text
                                else "Unknown error"
        ...
        if (
            partner.company_type == "person"
            and not partner.type == "invoice"
            and partner.parent_id
            and partner.parent_id.trip_customer
        ):
            first_name, last_name = (
                (partner.name.split(" ", 1) + [None])[:2]
                if isinstance(partner.name, str) and " " in partner.name
                else (partner.name, None)
            )
            contact_data = {
                "firstName": first_name,
                "lastName": last_name,
                ...
                "trip_customer": {"id": partner.parent_id.trip_customer}
                if partner.parent_id.trip_customer
                else None,
            }
            if contact_data:
                try:
                ...
                if response.status_code == 201:
                    ...
                    raise ValidationError(
                        _("Error creating contact in Tripletex: {}").format(
                            response.text if response.text else "Unknown error"
                        )
                    )
    ...
    def get_delivery_address(self):
        ...
        if (
            ...
            and not self.partner_shipping_id.type == "delivery"
        ):
            return delivery_address
        if (
            hasattr(self, "partner_shipping_id")
            and self.partner_shipping_id.type == "delivery"
            and not self.partner_shipping_id.trip_customer
        ):
            delivery_address = {
            ...
            self.partner_shipping_id.type == "delivery"
            and self.partner_shipping_id.trip_customer
        ):
        ...
        for line in self.invoice_line_ids:
            if line.display_type == "product":
                ...
                if not line.product_id:
                    raise ValidationError(
                        _("Missing product on invoice line: %s")
                        % line.display_name
                    )
                ...
                if not product_name.strip():
                    raise ValidationError(
                        _(
                            "Missing name/description on invoice line for product %s"
                        )
                        % line.product_id.display_name
                    )

                # Ensure product exists in Tripletex
                self._create_tripletex_product_if_needed(
                    base_url, line.product_id
                )
    ...
    def _create_tripletex_product_if_needed(self, base_url, product):
        ...
        if not product.tripletex_product:
            ...
            if response.status_code == 201:
                product.write(
                    {"tripletex_product": response.json()["value"]["id"]}
                )
            else:
                raise ValidationError(
                    _("Error creating Product in Tripletex: {}").format(
                        response.text if response.text else "Unknown error"
                    )
                )
